...
rectangle_data <- function(.data, col_name) {
    ...
    out <- .data %>%
        ...
        select(parkCode, name, all_of(col_name)) %>%
        #unnest the column name into its own columns
        unnest(col_name, names_sep = '_') %>%
        ...
        rename_at(vars(-parkCode, -name), ~paste0(col_name, '_', .)) %>%
        #return a flattened tibble
        flatten() %>%
        tibble() %>%
        #unnest again for deeper nested lists
        unnest()
...
get_park_info <- function(state = NULL, park_code = NULL, api_key = my_api_key, datasets = NULL, output_tibble = FALSE, endpoint = 'campgrounds') { 
    ...
    #if the user does not specify any datasets default to all of the options for that endpoint
    if(is.null(datasets)) {
        
        datasets <- switch(endpoint,
           'parks' = list('activities', 'topics', 'entranceFees', 'entrancePasses', 'operatingHours'),
           'campgrounds' = list('amenities', 'fees', 'operatingHours', 'campsites', 'accessibility'))
...
campground_data <- get_park_info(state = 'NC', endpoint = 'campgrounds', datasets = c('accessibility', 'fees', 'amenities'), output_tibble = FALSE)

park_data <- get_park_info(state = 'NC', endpoint = 'parks', output_tibble = FALSE) 
...
campground_national_data <- get_park_info(datasets = c('fees', 'accessibility'), output_tibble = FALSE)

accessibility <- campground_national_data$accessibility %>%
    select(campground = name, 
          # accessRoads = accessibility_accessibility_accessRoads,
           classifications = accessibility_accessibility_classifications) %>%
    mutate(dummy = 1) %>%
    group_by(campground) %>%
    pivot_wider(names_from =classifications, values_from = dummy, values_fn = max, values_fill = 0) %>%
   # mutate(dummy = 1) %>%
    #pivot_wider(names_from =accessRoads, values_from = dummy, values_fn = sum)


sum(duplicated(accessibility$campground))



roads <- campground_national_data$accessibility$accessibility_accessibility_classifications %>% unique()

fees_by_campground <- campground_national_data$fees %>%
    select(parkCode, campground = name, cost = fees_fees_cost) %>%
    mutate(cost = as.numeric(cost)) %>%
    group_by(parkCode, campground) %>%
    summarise(min = min(cost), avg = mean(cost), median = median(cost), max = max(cost))

sites_and_fees <- campground_national_data$data %>%
    select(parkCode, campground = name, numberOfSitesReservable, numberOfSitesFirstComeFirstServe, isPassportStampLocation) %>%
    mutate(reservable = as.numeric(numberOfSitesReservable), 
           walkup = as.numeric(numberOfSitesFirstComeFirstServe),
           total_sites = reservable + walkup) %>%
    inner_join(fees_by_campground, by = c('parkCode', 'campground')) %>%
    inner_join(accessibility, by = 'campground') %>%
    filter(avg < 400)
    

sites_and_fees_plot <- ggplot(data = sites_and_fees, aes(x = reservable, y = avg)) +
           geom_point(aes(color = `Developed Campground`)) +
